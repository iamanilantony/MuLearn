...
export default function Leaderboard() {
  ...
  const sortedData = [...dataToUse].sort((a, b) => b[activeFilter] - a[activeFilter]);
  const topPlayers = sortedData.slice(0, 3);
  const filteredData = sortedData.filter((entry) => !topPlayers.includes(entry));
  ...
  return (
    <div className={styles.container}>
      <div className={styles.leaderboard}>
        ...
        <LeaderboardTable leaderboardData={filteredData} filter={activeFilter} />
      </div>
    </div>
  );
}